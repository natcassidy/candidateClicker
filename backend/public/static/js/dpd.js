...
dpd.votes = dpd("/votes");
dpd.votes.on = function(ev, fn) {
  return dpd.on("votes" + ":" + ev, fn);
}
dpd.votes.once = function(ev, fn) {
  return dpd.once("votes" + ":" + ev, fn);
}
dpd.votes.off = function(ev, fn) {
  return dpd.off("votes" + ":" + ev, fn);
}
dpd.votes.rename = function (path, body, fn) {
  dpd.votes.exec("rename", path, body, fn);
}
